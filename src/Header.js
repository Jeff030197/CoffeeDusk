    ...
    <div className="relative overflow-hidden bg-hero-pattern bg-center bg-cover md:bg-top pb-32 max-w-screen-2xl mx-auto">
      ...
      <div className="relative max-w-screen-xl mx-auto">
        <button
          ...
          type="button"
          className="md:hidden mt-3 ml-3 rounded-md p-2 inline-flex items-center justify-center text-yellow-800 hover:text-yellow-900 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-yellow-500"
        >
        ...
        <div className="pb-8 sm:pb-16 md:pb-20 lg:max-w-2xl lg:w-full lg:pb-28 xl:pb-32">
              ...
              <div className="hidden md:block md:ml-10 md:pr-4 md:space-x-8">
                {props.navigation.map((nav) => (
                  <a
                    key={nav.name}
                    href={nav.href}
                    className="font-medium tracking-wider uppercase text-brown-500 hover:text-brown-700"
                  >
                    {nav.name}
                  </a>
                ))}
          ...
          <div className="ml-4 md:ml-12 px-4 md:px-8 mt-24 md:mt-36">
            <div className="text-left animate-fadein-bottom">
              ...
              Experience the perfect blend: freshly baked desserts and rich roasted coffee. Indulge in our exquisite treats and aromatic brews for a delightful taste sensation.
              </p>
              <Link
                to="/Menu"
                className="transition duration-500 ease-in-out w-max mt-6 sm:mt-8 tracking-wider flex items-center justify-center px-8 py-3 border-2 border-brown-500 font-medium rounded-md text-white bg-brown-500 hover:bg-transparent hover:text-brown-500 md:py-4 md:text-md md:px-10 uppercase"
              >
                Order now
              </Link>
            </div>
